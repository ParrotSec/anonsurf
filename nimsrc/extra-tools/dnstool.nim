...
proc writeTail(dnsAddr: string) =
  ...
  try:
    if sysResolvConf.access(W_OK) == F_OK:
      writeFile(tailResolvConf, dnsAddr)
    else:
      printErr("Can't write new data to " & sysResolvConf & ". Did you try sudo?")
  except:
